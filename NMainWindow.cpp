...
void NMainWindow::InitCentralWorkspace()
{
    ...
    if (_contentHost && _contentStack) {
        return;
    }

    _contentHost = root;
    _contentStack = new QStackedLayout(_contentHost);
    _contentStack->setContentsMargins(0, 0, 0, 0);
    _contentStack->setSpacing(0);
    _contentStack->setStackingMode(QStackedLayout::StackAll);

    _homeWorkspace = new MockMainWorkspace(_contentHost);
    _contentStack->addWidget(_homeWorkspace);
    _contentStack->setCurrentWidget(_homeWorkspace);
}
...
{
    if (_contentStack && _homeWorkspace) {
        _contentStack->setCurrentWidget(_homeWorkspace);
    }
